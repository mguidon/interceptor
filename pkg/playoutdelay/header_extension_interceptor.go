...
// NewInterceptor constructs a new HeaderExtensionInterceptor (implements interceptor.Factory)
func (h *HeaderExtensionInterceptorFactory) NewInterceptor(id string) (interceptor.Interceptor, error) {
	return &HeaderExtensionInterceptor{minDelay: h.minDelay, maxDelay: h.maxDelay}, nil
}

// NewHeaderExtensionInterceptor returns a HeaderExtensionInterceptorFactory
// minDelay and maxDelay are in units of 10ms granularity (e.g. 10*time.Millisecond = 1 unit)
func NewHeaderExtensionInterceptor(minDelay, maxDelay time.Duration) (*HeaderExtensionInterceptorFactory, error) {
	if minDelay.Milliseconds() < 0 || minDelay.Milliseconds() > playoutDelayMaxMs ||
		maxDelay.Milliseconds() < 0 || maxDelay.Milliseconds() > playoutDelayMaxMs {
		return nil, errPlayoutDelayInvalidValue
	}
	return &HeaderExtensionInterceptorFactory{
		minDelay: uint16(minDelay.Milliseconds() / 10),
		maxDelay: uint16(maxDelay.Milliseconds() / 10),
	}, nil
}
